...
function handle_attack_reputation_change(attacker, defender, AOE_flag) {
    if (AOE_flag)
        return;
    data_id_1.DataID.Reputation.set_a_X_b(defender.id, 'enemy', attacker.id);
}
...
function is_enemy_characters(character, target_character) {
    ...
    if (!(0, racial_hostility_1.hostile)(character.race, target_character.race)) {
        ...
        if (!data_id_1.DataID.Reputation.a_is_enemy_of_b(target_character.id, character.id) && !data_id_1.DataID.Reputation.a_is_enemy_of_b(character.id, target_character.id)) {
