...
export function craft_item(character: Character, item: ItemJson, durability: (character: Character) => number, skill: skill, tier: number) {
    let spear = ItemSystem.create(item)
    spear.durability = durability(character)
...
export function craft_bulk(character: Character, material: material_index, amount: (character: Character) => number, skill: skill, tier: number) {
    character.stash.inc(material, amount(character))
    Event.on_craft_update(character, skill, tier)
