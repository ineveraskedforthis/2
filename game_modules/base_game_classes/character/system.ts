import { Character, char_id } from "./character";
...
export namespace CharacterSystem {
    export function template_to_character(template: CharacterTemplate, name: string|undefined) {
        last_character_id = last_character_id + 1
        if (name == undefined) name = template.name_generator()
        let character = new Character(last_character_id, -1, -1, '#', name, template.archetype, template.stats, template.max_hp)
