...
module.exports = class Market {
    ...
    async clear_agent_orders(pool, agent, tag, save) {
        await this.clear_agent_buy_orders(pool, agent, tag, save, false);
        await this.clear_agent_sell_orders(pool, agent, tag, save, false);
        this.changed = true;
    }

    async clear_agent_sell_orders(pool, agent, tag, save) {
        var tmp = new Set();
        for (let i of this.sell_orders[tag]) {
        ...
        for (let i of tmp) {
            await this.cancel_sell_order(pool, i, save);
        }
    }

    async clear_agent_buy_orders(pool, agent, tag, save) {
        var tmp = new Set();
        for (let i of this.buy_orders[tag]) {
            let order = this.world.get_order(i);
            if (order.owner == agent) {
