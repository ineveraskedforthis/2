...
// handle tutorial
{
    ...
    if (localStorage.getItem("tutorial") != null) {
        ...
        function hide_thing(element: HTMLElement) {
            element.classList.remove("over-backdrop")
        }

        function unhighlight_thing(element: HTMLElement) {
            element.classList.remove("border-yellow")
            element.classList.remove("highlight-animation");
        }

        function attach_tutorial_callback(element: HTMLElement, tutorial_state_update: () => void) {
            saved_callback = element.onclick;
            function new_callback(ev: MouseEvent) {
