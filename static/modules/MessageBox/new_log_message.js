...
export function new_log_message(msg) {
    new_message_to_box('.messages_display.log', msg);
    console.log(msg);
}
export function new_message_to_box(target, log_message) {
    var log = select(target);
    console.log(log);
    for (const item of log) {
        var new_line = document.createElement('p');
        var text = document.createTextNode(log_message);
        if (target != '.messages_display.chat')
            new_line.style.whiteSpace = 'pre-line';
...
const damage_types = ['fire', 'blunt', 'pierce', 'slice'];
