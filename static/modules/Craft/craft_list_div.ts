...
import { elementById, elementParent, existsById, selectOne, selectOneFrom } from '../HTMLwrappers/common.js';

var craft_items : string[] = [];
var craft_bulk  : string[] = [];
...
function construct_craft_inputs(inputs: ItemAmountView[]) {
...
function new_craft_option(id: string): HTMLElement|undefined {
    if (existsById("c_" + id)) {
        return undefined
    }
...
function construct_craft_div(data: CraftBulkView) {
...
function construct_craft_item_div(data: CraftItemView) {
...
function update_craft_div(message: CraftBulkUpdateView) {
...
function update_craft_item_div(message: CraftItemUpdateView) {
    if (!existsById('c_' + message.tag)) {
        return
    }
