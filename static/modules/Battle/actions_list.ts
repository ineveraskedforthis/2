import { UnitSocket } from "@custom_types/battle_data.js";
...
const columns : Column<UnitSocket>[] = [
    {
        header_text: "Name",
...
export const enemies_list = new List<UnitSocket>(enemy_list_div);
enemies_list.columns = columns
enemies_list.onclick = (unit) => BattleImage.select(unit.id);
enemies_list.onmouseenter = (unit: UnitSocket, line: HTMLElement) => BattleImage.set_hover(unit.id, line);
enemies_list.onmouseleave = (unit: UnitSocket, line: HTMLElement) => BattleImage.remove_hover(unit.id, line);
